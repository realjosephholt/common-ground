...
| Data export and hard delete per user | Not just a compliance checkbox; leaving must actually be possible. |
...
1. **Sybil resistance is the central unsolved threat.** If accounts are free, "shared
   support" is meaningless and campaigns can be astroturfed. The planned mitigation is
   verification tiers plus a capped vouching graph, with vote weight scaled by tier.
   None of it is implemented yet, and none of it is sufficient against a determined,
   resourced adversary.
...
4. **Moderation is a censorship surface.** Whoever moderates can suppress causes. The
   mitigation is a public, appealable audit log; it is not built yet.
